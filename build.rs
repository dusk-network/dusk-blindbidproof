...
fn main() {
    ...
    let c = cbindgen::Builder::new()
        ...
        .with_header(format!("/* libblindbid Header Version {} */", VERSION))
    ...
    match c {
        Ok(res) => {
            res.write_to_file(target_path.join("libblindbid.h"));
        }
        Err(err) => {
    ...
    let mut pc_file = File::create(target_path.join("libblindbid.pc"))
    ...
    write!(
        ...
        "Name: blindbid
Version: {version}
Description: blindbid library library
Libs: {libs}
",
